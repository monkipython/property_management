    ...
    var createControls = function (that, header) {
        ...
        if (addedFilterControl) {
            header.off('keyup', 'input').on('keyup', 'input', function (event) {
                ...
                if ($.inArray(event.keyCode, [37,38,39,40,9,16,20,32,17,93,91,27,110,112,113,114,115,116,117,118,119,120,121,122,123,190]) > -1) {
                    return;
                }
    ...
    BootstrapTable.prototype.init = function () {
        ...
        if (this.options.filterControl) {
            ...
            this.$el.on('reset-view.bs.table', function () {
                ...
                // Not sure what is wrong with this. will come back later (Sean)
...
//        if (text) {
        ...
        $('.filter-control input,.filter-control select').each(function(){
          ...
          var fl = cls.replace(/form\-control bootstrap\-table\-filter\-control\-/, '');
          if(val){
            allFilter[fl] = val;
          }
        });
        ...
        for(var k in allFilter){
          this.filterColumnsPartial[k] = allFilter[k];
        }
        
        for(var k in this.filterColumnsPartial){
          if(allFilter[k] === undefined){
            delete this.filterColumnsPartial[k];
          }
        }
        
//          dd(this.valuesFilterControl)
//            this.filterColumnsPartial[$field] = text;
//            this.filterColumnsPartial['unit'] = '0002';
