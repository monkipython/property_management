<?php
namespace App\Http\Controllers\AccountReceivable\Service\ExportCsv;
...
class ServiceCsv {
  ...
  public static function getCsv($vData){
    $selectFields            = array_values(Helper::keyFieldName(DB::select('SHOW COLUMNS FROM ' . T::$service),'Field','Field'));
    $vData['selectedField']  = Helper::formElasticSelectedField($selectFields);
    $indexMain               = T::$serviceView . '/' . T::$serviceView . '/_search?';
    ...
    return Helper::exportCsv([
      'filename' => 'service_export_' . date('Y-m-d-H-i-s') . '.csv',
      'data'     => $returnData,
    ]);
  }
}