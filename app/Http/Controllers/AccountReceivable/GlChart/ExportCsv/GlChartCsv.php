<?php
namespace App\Http\Controllers\AccountReceivable\GlChart\ExportCsv;
...
class GlChartCsv {
  ...
  public static function getCsv($vData){
    $selectFields            = array_values(Helper::keyFieldName(DB::select('SHOW COLUMNS FROM ' . T::$glChart),'Field','Field'));
    $vData['selectedField']  = Helper::formElasticSelectedField($selectFields);
    $indexMain               = T::$glChartView . '/' . T::$glChartView . '/_search?';
    ...
    return Helper::exportCsv([
      'filename' => 'gl_chart_export_' . date('Y-m-d-H-i-s') . '.csv',
      'data'     => $returnData,
    ]);
  }
}