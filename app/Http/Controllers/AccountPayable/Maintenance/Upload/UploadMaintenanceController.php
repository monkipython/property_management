<?php
namespace App\Http\Controllers\AccountPayable\Maintenance\Upload;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Library\{Form, Html, Helper, Upload, V, File, TableName AS T};
use App\Http\Models\Model; // Include the models class
use Illuminate\Support\Facades\DB;
use App\Http\Models\AccountPayableModel AS M; // Include the models class

class UploadMaintenanceController extends Controller{
//------------------------------------------------------------------------------
  public function index(Request $req){
    ...
    //$r = M::getFileUpload(['vendor_maintenance_id'=>$vData['vendor_maintenance_id'], 'f.type'=>'maintenance']);
    $r = M::getFileUploadJoin(Model::buildWhere(['v.vendor_maintenance_id'=>$vData['vendor_maintenance_id'],'f.type'=>'maintenance']),T::$vendorMaintenance,'vendor_maintenance_id');
    return Upload::getViewlistFile($r, '/uploadMaintenance','upload',['includeDeleteIcon'=>1]);
  }
//------------------------------------------------------------------------------  
  public function show($id, Request $req){
    ...
    $r = M::getFileUploadJoin(Model::buildWhere(['f.uuid'=>$id]),T::$vendorMaintenance,'vendor_maintenance_id',1,['f.type','f.file','f.uuid','f.ext']);
    //$r = M::getFileUpload(Model::buildWhere(['uuid'=>$id]), 1,['f.type','f.file','f.uuid','f.ext']);
    if(preg_match('/^old_/', $id)){
      $v['data']['path'] = File::getLocation('Maintenance')['showUpload'] . implode('/', [$r['type'], $r['file']]);
    } else{
      $v['data']['path'] = File::getLocation('Maintenance')['showUpload'] . implode('/', [$r['type'], $r['uuid'], $r['file']]);
    }
    $v['data']['ext']  = $r['ext'];
    $vData = $v['data'];
    if($req->ajax()) {
  ...
  public function store(Request $req){
    ...
    try{
      ...
      if(!empty($insertData['foreign_id'])){
        $elastic = ['insert'=>[
          T::$vendorMaintenanceView=>['vm.vendor_maintenance_id'=>[$vData['data']['foreign_id']]]
        ]];
...
//------------------------------------------------------------------------------
  public function destroy($id, Request $req){
    ...
    try{
      ...
      $elastic            = ['insert'=>[T::$vendorMaintenanceView=>['vm.vendor_maintenance_id'=>[$r['foreign_id']]]]];
