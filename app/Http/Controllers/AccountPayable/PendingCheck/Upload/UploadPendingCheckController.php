<?php
namespace App\Http\Controllers\AccountPayable\PendingCheck\Upload;
...
class UploadPendingCheckController extends Controller{
  ...
  public function index(Request $req){
    ...
    //$r = M::getFileUpload(['vendor_pending_check_id'=>$vData['vendor_pending_check_id'], 'f.type'=>'pending_check']);
    $r     = M::getFileUploadJoin(Model::buildWhere(['v.vendor_pending_check_id'=>$vData['vendor_pending_check_id'],'f.type'=>'pending_check']),T::$vendorPendingCheck,'vendor_pending_check_id',0,['v.vendor_pending_check_id','f.uuid','f.file','f.path','f.ext','f.name','f.type']);
    return Upload::getViewlistFile($r, '/uploadPendingCheck');
  }
//------------------------------------------------------------------------------  
  public function show($id, Request $req){
    ...
    $r = M::getFileUploadJoin(Model::buildWhere(['f.uuid'=>$id]),T::$vendorPendingCheck,'vendor_pending_check_id',1,['f.type','f.file','f.uuid','f.ext']);
    if(preg_match('/^old_/', $id)){
      $v['data']['path'] = File::getLocation('PendingCheck')['showUpload'] . implode('/', [$r['type'], $r['file']]);
    } else{
      $v['data']['path'] = File::getLocation('PendingCheck')['showUpload'] . implode('/', [$r['type'], $r['uuid'], $r['file']]);
    }
    $v['data']['ext']  = $r['ext'];
    $vData = $v['data'];
    if($req->ajax()) {
...
//------------------------------------------------------------------------------
  public function store(Request $req){
    ...
    try{
      ...
      if(!empty($insertData[T::$fileUpload]['foreign_id'])){
        $elastic = ['insert'=>[
          T::$vendorPendingCheckView=>['vp.vendor_pending_check_id'=>[$vData['data']['foreign_id']]]
        ]];
...
//------------------------------------------------------------------------------
  public function destroy($id, Request $req){
    ...
    try{
      ...
      $commit = [
        'success'=>$success, 
        'elastic'=>['insert'=>[T::$vendorPendingCheckView=>['vp.vendor_pending_check_id'=>[$r['foreign_id']]]]]
      ];
