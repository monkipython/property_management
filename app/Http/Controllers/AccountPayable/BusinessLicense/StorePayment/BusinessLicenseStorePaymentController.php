<?php
namespace App\Http\Controllers\AccountPayable\BusinessLicense\StorePayment;
...
/*
UPDATE `ppm`.`accountProgram` SET `subController` = 'accountPayableBankInfo,uploadBusinessLicense,uploadApproval,businessLicensePaymentStore' WHERE (`module` = 'businessLicense');
 */

class BusinessLicenseStorePaymentController extends Controller {
  private $_viewPath   = 'app/AccountPayable/BusinessLicense/paymentStore/';
  private $_viewTable  = '';
  public function __construct(){
    $this->_viewTable   = T::$vendorBusinessLicenseView;
  }
//------------------------------------------------------------------------------
  public function create(Request $req){
    ...
    $r            = !empty($vData['vendor_payment_id']) ? M::getVendorPaymentElastic(Helper::selectData(['vendor_payment_id','invoice_date'],$vData) + ['type'=>'business_license'],
    ...
    $rLicense     = M::getBusinessLicenseElastic(['vendor_util_payment_id'=>$vData['vendor_util_payment_id']],['vendor_util_payment_id','invoice','prop','vendid'],1);
    $r          += ['vendor_util_payment_id'=>$vData['vendor_util_payment_id'],'uuid'=>'','field'=>$vData['field']] + Helper::selectData(['prop','invoice','vendid'],$rLicense);
    $uploadFiles = [];
    
    $fileUpload  = Helper::getValue('fileUpload',$r,[]);
    foreach($fileUpload as $v){
      if($v['type'] == 'Approval' || $v['type'] == 'approval'){
        $uploadFiles[] = $v;
        $r['uuid'] .= $v['uuid'] . ',';
      }
    }
    $fileUploadList = Upload::getViewlistFile($uploadFiles, '/uploadBusinessLicense','upload',['includeDeleteIcon'=>1]);
...
//------------------------------------------------------------------------------
  public function store(Request $req){
    ...
    $field       = $vData['field'];
    $verifyR     = $this->_verifyInvoiceDate($id,$invoiceDate,$field);
    if(!empty($verifyR)){
      return $verifyR;
    }
    $paymentId   = !empty($vData['vendor_payment_id']) ? $vData['vendor_payment_id'] : 0;
    $uuid        = !empty($vData['uuid']) ? explode(',',(rtrim($vData['uuid'],','))) : [];
    $rLicense    = M::getBusinessLicenseElastic(['vendor_util_payment_id'=>$id],['vendor_util_payment_id','vendor_id','prop','unit','gl_acct','vendid'],1);
    $r           = M::getVendorPaymentElastic(['vendor_payment_id'=>$paymentId],['vendor_payment_id','foreign_id','vendor_id','remark','vendid','prop','unit','bank','type','gl_acct','check_pdf',T::$fileUpload],1);
    ...
    if(!empty($r)){
      $updateData += [
        T::$vendorPayment => [
          ...
          'updateData'  => Helper::selectData(['usid','remark','invoice','invoice_date','amount'],$vData) + ['foreign_id'=>$id,'type'=>'business_license'],
        ]
      ];
    } else {
      $rGlChart      = Helper::keyFieldName(HelperMysql::getGlChat(Model::buildWhere(['g.prop'=>$rLicense['prop']])),'gl_acct');
      $rService      = Helper::keyFieldName(HelperMysql::getService(Model::buildWhere(['prop'=>$rLicense['prop']])),'service');
      $data          = Helper::selectData(['vendor_id','prop','unit','gl_acct','vendid'],$rLicense) 
                        + Helper::selectData(['usid','remark','invoice','invoice_date','amount'],$vData) 
                        + ['foreign_id'=>$id,'type'=>'business_license','tenant'=>0];
    ...
    try {
      ...
      $success       += !empty($success['insert:'.T::$vendorPayment]) ? [] : [];
      $elastic        = [
        'insert'     => [
          $this->_viewTable      => ['vu.vendor_util_payment_id'=>[$id]],
...
//------------------------------------------------------------------------------
  private function _getButton($fn,$req=[]){
    ...
    $button = [
      ...
      'edit'            => ['submit'=>['id'=>'submit','value'=>'Update Business License','class'=>'col-sm-12']],
    ];
    return $button[$fn];
  }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
  private function _getErrorMsg($name,$param=[]){
    $data = [
      '_verifyInvoiceDate' => Html::errMsg(date('Y',strtotime(Helper::getValue('invoice_date',$param))).' check is already issued for this vendor. Please try different year.'),
    ];
    return $data[$name];
  }
################################################################################
...
//------------------------------------------------------------------------------
  private function _getLatestPayment($id,$invoiceDate=''){
    $r                = M::getBusinessLicenseElastic(['vendor_util_payment_id'=>$id],['vendor_util_payment_id','remark',T::$vendorPayment],1);  
...
//------------------------------------------------------------------------------
  private function _verifyInvoiceDate($id,$invoiceDate,$field){
    $year         = preg_replace('/vendor_payment_hidden_date_field_/','',$field);
    $enteredYear  = date('Y',strtotime($invoiceDate));
    $pastDate     = date('Y-01-01',strtotime($invoiceDate));
    $futureDate   = date('Y-12-31',strtotime($invoiceDate));
