<?php
namespace App\Http\Controllers\AccountPayable\BusinessLicense;
...
/*
UPDATE `ppm`.`accountProgram` SET `subController` = 'businessLicenseedit,businessLicenseupdate,businessLicensecreate,businessLicensestore,accountPayableBankInfo,uploadBusinessLicense,paymentStoreBusinessLicense' WHERE (`accountProgram_id` = '226');UPDATE `ppm`.`accountProgram` SET `subController` = 'businessLicenseedit,businessLicenseupdate,businessLicensecreate,businessLicensestore,accountPayableBankInfo,uploadBusinessLicense,paymentStoreBusinessLicense' WHERE module='businessLicense';
 */
class BusinessLicenseController  extends Controller {
  private $_viewPath          = 'app/AccountPayable/BusinessLicense/businessLicense/';
  private $_viewTable         = '';
  private $_indexMain         = '';
...
//------------------------------------------------------------------------------
  public function edit($id,Request $req){
    ...
    $r      = M::getBusinessLicenseElastic(['vendor_util_payment_id'=>$id],[],1);
...
//------------------------------------------------------------------------------
  public function update($id,Request $req){
    ...
    $r                    = M::getBusinessLicenseElastic(['vendor_util_payment_id'=>$id],['vendor_util_payment_id','prop','gl_acct','vendid'],1);
    ...
    try{
      ...
      $elastic = [
        'insert'=>[$this->_viewTable=>['vu.vendor_util_payment_id'=>[$id]]]
      ];
      Model::commit([
        'success' =>$success,
        'elastic' =>$elastic
...
//------------------------------------------------------------------------------
  public function store(Request $req){
    ...
    try{
      ...
      $elastic = [
        'insert'=>[$this->_viewTable=>['vu.vendor_util_payment_id'=>[$insertedId]]]
      ];
      $response['vendorBusinessLicenseId'] = $insertedId;
...
//------------------------------------------------------------------------------
  public function destroy($id, Request $req) {
    ...
    try{
      ...
      $commit['elastic']['delete'] = [T::$vendorBusinessLicenseView => ['vendor_util_payment_id'=>$deleteIds]];
...
//------------------------------------------------------------------------------
  private function _getButton($fn,$req=[]){
    ...
    $button = [
      'create'          => ['submit'=>['id'=>'submit','value'=>'Create Business License','class'=>'col-sm-12']],
...
//------------------------------------------------------------------------------
  private function _getOrderField($fn,$req=[]){
    $perm         = Helper::getPermission($req);
    $orderField   = [
      'create'          => ['vendid','prop','gl_acct','invoice','due_date','remark'],
...
//------------------------------------------------------------------------------
  private function _getSuccessMsg($name){
    $data = [
      ...
      'destroy' =>Html::sucMsg('Successfully Deleted Business License(s)'),
    ];
    return $data[$name];
  }
//------------------------------------------------------------------------------
  private function _getActionIcon($perm){
    $actionData   = [];
    $actionData[] = '<a class="edit" href="javascript:void(0)" title="Edit"><i class="fa fa-edit text-aqua pointer tip" title="Edit Business License Information"></i></a>';
    $num          = count($actionData);
    return ['width'=>$num * 42,'icon'=>$actionData];
  }
//------------------------------------------------------------------------------
  private function _addDateColumns($data){
    $nowTs       = strtotime(date('Y'). ' +1 year');
    $pastTs      = strtotime(date('Y') . ' -6 year');
    $currentTs   = $nowTs;
    $u = 0;
    $numCols     = 8;
    while($currentTs > $pastTs && $u++ < $numCols){
      $field     = T::$vendorPayment . '_hidden_date_field_' . date('Y',$currentTs);
      $code      = 'Yr-' . date('Y',$currentTs);
      $col       = ['field'=>$field,'title'=>$code];
      $col      += $u == ($numCols-1) ? [] : ['width'=>50];
      $data[]    = $col;
      $curDate   = date('M-Y',$currentTs);
      $currentTs = strtotime($curDate . ' -1 year');
    }
    return $data;
  }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
  private function _gatherAggregations($r){
    ...
    foreach($rPayment as $i => $v){
      ...
      $code          = T::$vendorPayment . '_hidden_date_field_' . date('Y',strtotime($invoiceDate));
      $r[$code]       = $v;
      $codes[$code]   = $code;
      $printed[$code] = $print;
    }
    ...
    foreach($dateCols as $f){
      $r[$f]   = !empty($r[$f]) ? $r[$f] : Html::a('Empty',[
          ...
          'data-hidden-row-id' => $r['vendor_util_payment_id'],
          'data-hidden-id'     => 0,
          'data-hidden-date'   => date(preg_replace('/vendor_payment_hidden_date_field_/','',$f) . '-m-d'),
          'data-hidden-value'  => 0,
