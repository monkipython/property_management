<?php
namespace App\Http\Controllers\AccountPayable\Insurance\Upload;
...
class UploadInsuranceController extends Controller{
  public function show($id, Request $req){
    ...
    $id = empty($id) ? $v['data']['uuid'] : $id;
    $r  = DB::table(T::$fileUpload)->where('uuid', '=', $id)->first();
    if(preg_match('/^old_/', $id)){
      $v['data']['path'] = File::getLocation('Insurance')['showUpload'] . implode('/', [$r['type'], $r['file']]);
    }else{
      $v['data']['path'] = File::getLocation('Insurance')['showUpload'] . implode('/', [$r['type'], $r['uuid'], $r['file']]);
    }
    $v['data']['ext']  = $r['ext'];
    $vData = $v['data'];
    if($req->ajax()) {
  ...
  public function store(Request $req){
    ...
    try{
      ...
      if(!empty($insertData['foreign_id'])){
        $elastic = ['insert'=>[
          T::$vendorInsurance=>['vi.vendor_insurance_id'=>[$vData['data']['foreign_id']]]
        ]];
...
//------------------------------------------------------------------------------
  public function destroy($id, Request $req){
    ...
    try{
      ...
      $commit = [
        'success'=>$success, 
        'elastic'=>['insert'=>[T::$vendorInsuranceView=>['vi.vendor_insurance_id'=>[$r['foreign_id']]]]]
      ];
