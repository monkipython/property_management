<?php
namespace App\Http\Controllers\AccountPayable\PropTax\Upload;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Library\{Html, Upload, V, File, TableName AS T};
use App\Http\Models\Model; // Include the models class
use Illuminate\Support\Facades\DB;

class UploadPropTaxController extends Controller{
  ...
  public function show($id, Request $req){
    ...
    if(preg_match('/^old_/', $id)){
      $v['data']['path'] = File::getLocation('PropTax')['showUpload'] . implode('/', [$r['type'], $r['file']]);
    } else{
      $v['data']['path'] = File::getLocation('PropTax')['showUpload'] . implode('/', [$r['type'], $r['uuid'], $r['file']]);
    }
    $v['data']['ext']  = $r['ext'];
    $vData = $v['data'];
 
    return ($req->ajax()) ? ['html'=>Upload::getViewContainerType($vData['path'])]
                          : Html::tag('object','', ['data'=>env('APP_URL') . $vData['path'], 'width'=>'100%', 'height'=>'98%']) ;
  ...
  public function store(Request $req){
    ...
    try{
      ...
      if(!empty($insertData['foreign_id'])){
        $elastic = ['insert'=>[
          T::$vendorPropTax=>['pt.vendor_prop_tax_id'=>[$vData['data']['foreign_id']]]
        ]];
...
//------------------------------------------------------------------------------
  public function destroy($id, Request $req){
    ...
    try{
      ...
      $commit = [
        'success'=>$success, 
        'elastic'=>['insert'=>[T::$vendorPropTaxView=>['pt.vendor_prop_tax_id'=>[$r['foreign_id']]]]]
      ];
