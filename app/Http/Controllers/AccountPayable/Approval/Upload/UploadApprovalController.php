<?php
namespace App\Http\Controllers\AccountPayable\Approval\Upload;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Library\{Html, Helper, Elastic, Upload, V, File, TableName AS T};
...
//------------------------------------------------------------------------------
  public function index(Request $req){
    ...
    $r     = Helper::getValue('_source',Helper::getElasticResult(Elastic::searchQuery([
      ...
      '_source'   => ['vendor_payment_id',T::$fileUpload],
      'query'     => [
        'must'    => [
          'vendor_payment_id' => $vData['vendor_payment_id'],
        ]
      ]
    ]),1),[]);
    $r     = Helper::getValue(T::$fileUpload,$r,[]);
...
//------------------------------------------------------------------------------  
  public function show($id, Request $req){
    ...
    if(preg_match('/^old_/', $id)){
      $v['data']['path'] = File::getLocation('Approval')['showUpload'] . implode('/', [$r['type'], $r['file']]);
    } else{
      $v['data']['path'] = File::getLocation('Approval')['showUpload'] . implode('/', [$r['type'], $r['uuid'], $r['file']]);
    }
    $v['data']['ext']  = $r['ext'];
    $vData = $v['data'];
    if($req->ajax()) {
  ...
  public function store(Request $req){
    ...
    try{
      ...
      if(!empty($insertData['foreign_id'])){
        $elastic = ['insert'=>[
          T::$vendorPaymentView=>['vp.vendor_payment_id'=>[$vData['data']['foreign_id']]]
        ]];
...
//------------------------------------------------------------------------------
  public function destroy($id, Request $req){
    ...
    try{
      ...
      $commit = [
        'success'=>$success, 
        'elastic'=>['insert'=>[T::$vendorPaymentView=>['vp.vendor_payment_id'=>[$r['foreign_id']]]]]
      ];
