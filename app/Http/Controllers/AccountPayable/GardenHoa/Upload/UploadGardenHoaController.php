<?php
namespace App\Http\Controllers\AccountPayable\GardenHoa\Upload;
...
class UploadGardenHoaController extends Controller{
//------------------------------------------------------------------------------
  public function index(Request $req){
    ...
    //$r = M::getFileUpload(['vendor_gardenHoa_id'=>$vData['vendor_gardenHoa_id'], 'f.type'=>'gardenHoa']);
    $r     = M::getFileUploadJoin(Model::buildWhere(['v.vendor_gardenHoa_id'=>$vData['vendor_gardenHoa_id'],'f.type'=>'gardenHoa']),T::$vendorGardenHoa,'vendor_gardenHoa_id');
    return Upload::getViewlistFile($r, '/uploadGardenHoa');
  }
//------------------------------------------------------------------------------  
  public function show($id, Request $req){
    ...
    $r = M::getFileUploadJoin(Model::buildWhere(['f.uuid'=>$id]),T::$vendorGardenHoa,'vendor_gardenHoa_id',1,['f.type','f.file','f.uuid','f.ext']);
    //$r = M::getFileUpload(Model::buildWhere(['uuid'=>$id]), 1,['f.type','f.file','f.uuid','f.ext']);
    if(preg_match('/^old_/', $id)){
      $v['data']['path'] = File::getLocation('GardenHoa')['showUpload'] . implode('/', [$r['type'], $r['file']]);
    } else{
      $v['data']['path'] = File::getLocation('GardenHoa')['showUpload'] . implode('/', [$r['type'], $r['uuid'], $r['file']]);
    }
    $v['data']['ext']  = $r['ext'];
    $vData = $v['data'];
    if($req->ajax()) {
  ...
  public function store(Request $req){
    ...
    try{
      ...
      if(!empty($insertData['foreign_id'])){
        $elastic = ['insert'=>[
          T::$vendorGardenHoaView=>['vg.vendor_gardenHoa_id'=>[$vData['data']['foreign_id']]]
        ]];
...
//------------------------------------------------------------------------------
  public function destroy($id, Request $req){
    ...
    try{
      ...
      $elastic            = ['insert'=>[T::$vendorGardenHoaView=>['vg.vendor_gardenHoa_id'=>[$r['foreign_id']]]]];
